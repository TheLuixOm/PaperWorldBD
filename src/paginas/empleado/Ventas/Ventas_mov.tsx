...
function Ventas_mov() {
    ...
    const totalVenta = itemsVenta.reduce((acumulado, item) => {
        ...
        return (
                <section className="inventarioVista ventas_escMov" id="ventas"> 
                    <header className='encabezadoMov'>  
                <h2 className="VentasTituloMov">Ventas</h2>
                <div className="busqueda-wrapMov">
                    ...
                    {mostrarSugerencias && (
                        <ul className="lista-busquedaMov">
                            {vista_productos.map((p, i) => (
                                <li
                                    key={p.id}
                                    className={i === indiceActivo ? 'activoMov' : ''}
                                    onMouseDown={() => {
                                    setIndiceActivo(i);
                ...
                <UsuarioMenu className="inventarioUsuarioMenu" ariaLabel="Perfil del usuario" />
                    ...
                    <div className="panel-detalleMov">
                        
                            {productoSeleccionado ? (

                                <div className="detalle-productoMov">
                ...
                <h3 className='VentasSubtituloMov'>Lista de productos seleccionados</h3>
                <div className="lista-ventaMov">
                    
                    {itemsVenta.length === 0 ? (
                        null
                    ) : (
                        <>
                            <ul className="lista-venta-itemsMov">
                                {itemsVenta.map((item) => (
                                    <li key={item.producto.id} className="item-ventaMov">
                                        <div className="item-venta-infoMov">
                                        ...
                                        <div className="controles-cantidadMov">
                                            ...
                                            <span className="cantidad-chipMov">{item.cantidad}</span>
                                            <button type="button" onClick={() => actualizarCantidad(item.producto.id, 1)}>+</button>
                                        </div>
                                    </li>
                                ))}
                ...
                <h3 className='VentasSubtituloMov'>Total</h3>
                <div className="total-compraMov">
                    <div className="total-compra-resumenMov">
